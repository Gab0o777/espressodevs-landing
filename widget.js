 ...
 *   data-title       header name + welcome-card heading (default: "Umeia")
 *   data-subtitle    small line under the header name (default: "En línea")
 *   data-description welcome-card paragraph (default: "¿En qué te puedo ayudar hoy?")
 *
...
(function () {
  ...
  var css = [
    ...
    "  width: 60px; height: 60px; border-radius: 50%; border: none; cursor: pointer; padding: 0; overflow: hidden;",
    ...
    ".umeia-bubble svg { width: 26px; height: 26px; fill: #fff; }",
    ".umeia-panel {",
    "  position: fixed; bottom: 92px; " + POSITION + ": 20px; z-index: 2147483000;",
    "  width: 340px; max-width: calc(100vw - 40px); height: 520px; max-height: calc(100vh - 140px);",
    "  background: #fff; border-radius: 20px; box-shadow: 0 12px 40px rgba(20,10,50,0.22);",
    "  display: none; flex-direction: column; overflow: hidden;",
    "}",
    ".umeia-panel.umeia-open { display: flex; }",
    ".umeia-header {",
    "  background: #fff; color: #16151a; padding: 14px 14px 14px 16px;",
    "  display: flex; align-items: center; justify-content: space-between; gap: 8px;",
    "  border-bottom: 1px solid #efedf5; flex-shrink: 0;",
    "}",
    ".umeia-header-left { display: flex; align-items: center; gap: 10px; min-width: 0; }",
    ".umeia-avatar-wrap { position: relative; width: 38px; height: 38px; flex-shrink: 0; }",
    ".umeia-avatar-wrap img { width: 38px; height: 38px; border-radius: 50%; object-fit: cover; display: block; }",
    ".umeia-online-dot {",
    "  position: absolute; right: -1px; bottom: -1px; width: 11px; height: 11px;",
    "  border-radius: 50%; background: #2ecc71; border: 2px solid #fff;",
    "}",
    ".umeia-header-text { min-width: 0; }",
    ".umeia-header-name { font-weight: 700; font-size: 14.5px; color: #16151a; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }",
    ".umeia-header-subtitle { font-size: 12px; color: #8b8996; margin-top: 1px; }",
    ".umeia-close {",
    "  background: transparent; border: none; color: #9a98a5; font-size: 20px; cursor: pointer;",
    "  line-height: 1; padding: 4px 8px; border-radius: 8px; flex-shrink: 0;",
    "}",
    ".umeia-close:hover { background: #f4f2fa; color: #16151a; }",
    ".umeia-messages { flex: 1; overflow-y: auto; padding: 6px 14px 14px; display: flex; flex-direction: column; gap: 12px; background: #f7f6fb; }",
    ".umeia-welcome { text-align: center; padding: 22px 14px 16px; flex-shrink: 0; }",
    ".umeia-welcome-avatar-wrap {",
    "  position: relative; width: 68px; height: 68px; margin: 0 auto 12px;",
    "  display: flex; align-items: center; justify-content: center;",
    "}",
    ".umeia-welcome-avatar-wrap::before {",
    "  content: ''; position: absolute; inset: -16px; border-radius: 50%;",
    "  background: radial-gradient(circle, color-mix(in srgb, " + ACCENT_COLOR + " 28%, transparent) 0%, transparent 72%);",
    "}",
    ".umeia-welcome-avatar-wrap img {",
    "  position: relative; width: 68px; height: 68px; border-radius: 20px; object-fit: cover;",
    "  box-shadow: 0 8px 20px rgba(20,10,50,0.18);",
    "}",
    ".umeia-welcome h2 { margin: 0 0 6px; font-size: 16px; font-weight: 700; color: #16151a; }",
    ".umeia-welcome p { margin: 0; font-size: 13px; line-height: 1.5; color: #7b7987; padding: 0 8px; }",
    ...
    ".umeia-caption { font-size: 11px; color: #a09eab; margin-top: 4px; padding: 0 4px; }",
  ...
  bubble.innerHTML = '<img src="' + LOGO_SRC + '" alt="" />';
  ...
  panel.innerHTML =
    ...
    '  <div class="umeia-welcome">' +
    '    <div class="umeia-welcome-avatar-wrap"><img src="' + LOGO_SRC + '" alt="" /></div>' +
    "    <h2>" + TITLE + "</h2>" +
    "    <p>" + DESCRIPTION + "</p>" +
    "  </div>" +
    "</div>" +
  ...
  function renderAll() {
    // The welcome card is a permanent fixture, not part of the replayed
    // transcript — every other child gets cleared and rebuilt on reload.
    var children = Array.prototype.slice.call(messagesEl.children);
    children.forEach(function (child) {
      if (!child.classList.contains("umeia-welcome")) child.remove();
    });
